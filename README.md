...
- [ ] 생성자들은 단순한 값을 할당하는 것이지만 검증 로직이 추가된다면 어떨까?
- [ ] equals와 hashCode를 사용하지 않는데 구현된 이유가 무엇일까?
- [ ] DTO 에서 DB에서 양수만 저장한다라는 것을 꼭 알아야 할까? DTO는 값이 있어야 한다는 정도만 관리해도 될 것 같다. 
- [ ] 노선과 역의 이름은 최대 255의 문자열인데 이것을 검증하는 것을 DTO와 도메인 중 어디에 있는게 적절할지 조금 더 고민해보면 좋을 것 같다.
