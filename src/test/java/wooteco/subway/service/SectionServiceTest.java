...
public class SectionServiceTest {
    ...
    void save_not_change_up() {
        ModifyResult result = sectionService.save(new SectionRequest(1L, 4L, 10L), 1L);
        assertAll(() -> assertThat(result.getSaveResult().size()).isEqualTo(2),
                () -> assertThat(result.getDeleteResult().size()).isEqualTo(1));
    }

    @DisplayName("상행선을 바꾸고 거리 조건을 만족하는 구간을 등록할 때 정확히 저장하는지 테스트")
    @Test
    void save_change_up() {
        ModifyResult result = sectionService.save(new SectionRequest(4L, 1L, 10L), 1L);
        assertAll(() -> assertThat(result.getSaveResult().size()).isEqualTo(1),
                () -> assertThat(result.getDeleteResult().size()).isEqualTo(0));
    }

    @DisplayName("하행선을 바꾸지 않고 거리 조건을 만족하는 구간을 등록할 때 정확히 저장하는지 테스트")
    @Test
    void save_not_change_down() {
        ModifyResult result = sectionService.save(new SectionRequest(4L, 3L, 10L), 1L);
        assertAll(() -> assertThat(result.getSaveResult().size()).isEqualTo(2),
                () -> assertThat(result.getDeleteResult().size()).isEqualTo(1));
    }
    ...
    void save_change_down() {
        ModifyResult result = sectionService.save(new SectionRequest(3L, 4L, 10L), 1L);
        assertAll(() -> assertThat(result.getSaveResult().size()).isEqualTo(1),
                () -> assertThat(result.getDeleteResult().size()).isEqualTo(0));
    }
    ...
    @Test
    void save_cycle() {
        ModifyResult result = sectionService.save(new SectionRequest(3L, 1L, 10L), 1L);
        assertAll(() -> assertThat(result.getSaveResult().size()).isEqualTo(1),
                () -> assertThat(result.getDeleteResult().size()).isEqualTo(0));
    }
