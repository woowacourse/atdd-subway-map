...
public class SectionServiceTest {
    ...
    void save_not_change_down_end_dist_over() {
        assertThatThrownBy(() -> sectionService.enroll(new Section(1L, 4L, 2L, 100L)))
    ...
    void save_not_change_up_end_dist_over() {
        assertThatThrownBy(() -> sectionService.enroll(new Section(1L, 4L, 3L, 100L)))
                .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    @Test
    void save_exist() {
        assertThatThrownBy(() -> sectionService.enroll(new Section(1L, 2L, 3L, 10L)))
                .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    @Test
    void save_sub_cycle() {
        assertThatThrownBy(() -> sectionService.enroll(new Section(1L, 3L, 2L, 10L)))
                .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    @Test
    void save_no_exist() {
        assertThatThrownBy(() -> sectionService.enroll(new Section(1L, 4L, 5L, 10L)))
                .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void save_not_change_up() {
        Section result = sectionService.enroll(new Section( 1L, 1L, 4L, 10L));
        assertThat(sectionDao.findById(result.getId())).isNotNull();
    }

    @DisplayName("상행선을 바꾸고 거리 조건을 만족하는 구간을 등록할 때 정확히 저장하는지 테스트")
    @Test
    void save_change_up() {
        Section result = sectionService.enroll(new Section(1L, 4L, 1L, 10L));
        assertThat(sectionDao.findById(result.getId())).isNotNull();
    }
    ...
    void save_not_change_down() {
        Section result = sectionService.enroll(new Section(1L, 4L, 3L, 10L));
        assertThat(sectionDao.findById(result.getId())).isNotNull();
    }
    ...
    void save_change_down() {
        Section result = sectionService.enroll(new Section(1L, 3L, 4L, 10L));
        assertThat(sectionDao.findById(result.getId())).isNotNull();
    }
    ...
    @Test
    void save_cycle() {
        Section result = sectionService.enroll(new Section(1L, 3L, 1L, 10L));
        assertThat(sectionDao.findById(result.getId())).isNotNull();
    }
    ...
    @Test
    void delete_cycle() {
        sectionService.enroll(new Section(1L, 3L, 1L, 10L));
