export const listItemTemplate = (station) =>
  `<div data-station-id="${station.id}" class="list-item border border-gray-200 py-2 px-4 text-gray-800 station-${station.id}">
    ${station.name}
...
export const subwayLinesTemplate = (line, id) =>
  `<div data-line-id="${id}" class="subway-line-item border border-gray-200 py-2 px-4 text-gray-800" id="${id}">
...
export const optionTemplate = (lineId, lineName) => `<option data-line-id="${lineId}">${lineName}</option>`;
  ...
  <div class="flex items-center flex-shrink-0 text-gray-800 w-full">
    ...
    <div class="flex justify-start">
      <div class="hover:bg-yellow-400 px-2 py-1 rounded">
         <a href="/templates/admin-station.html" class="block inline-block lg:mt-0 text-gray-800 text-sm">
          역 관리
          </a>
      </div>
      <div class="hover:bg-yellow-400 px-2 py-1 rounded">
         <a href="/templates/admin-line.html" class="block inline-block lg:mt-0 text-gray-800 text-sm">
          노선 관리
          </a>
      </div>
      <div class="hover:bg-yellow-400 px-2 py-1 rounded">
          <a href="/templates/admin-edge.html" class="block inline-block lg:mt-0 text-gray-800 text-sm">
          구간 관리
          </a>
      </div>
    </div>
...
export const subwayLinesItemTemplate = line => {
  ...
  return `<div class="inline-block w-1/2 px-2">
            <div class="rounded-sm w-full slider-list">
              ...
              <div class="overflow-y-auto height-90 station-container station-container-${line.id}" data-line-id="${line.id}">
              ${stationsTemplate}
              </div>
            </div>
          </div>`;
