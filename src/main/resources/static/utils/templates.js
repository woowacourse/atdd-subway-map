...
export const subwayLinesTemplate = line =>
  `<div class="subway-line-item border border-gray-200 py-2 px-4 text-gray-800" data-line-id="${line.id}">
      ...
      ${line.name}
      <button class="hover:bg-gray-300 hover:text-gray-500 text-gray-300 px-1 rounded-full float-right">
         <span class="modal-open mdi mdi-delete"></span>
      </button>
      <button class="hover:bg-gray-300 hover:text-gray-500 text-gray-300 px-1 rounded-full float-right">
         <span class="modal-open mdi mdi-pencil"></span>
      </button>
    </div>`;

export const optionLineTemplate = value => `<option data-option-line-id="${value.id}">${value.name}</option>`;
