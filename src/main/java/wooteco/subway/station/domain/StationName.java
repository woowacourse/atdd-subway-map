...
public class StationName {
    ...
    private void validateNameLength(String name) {
        if (name.length() > MAX_NAME_LENGTH) {
            throw new WrongStationInfoException(String.format("역 이름은 %d자를 초과할 수 없습니다. 이름의 길이 : %d", MAX_NAME_LENGTH, name.length()));
        }
    }

    private void validateInvalidName(String name) {
        if (!VALID_NAME_PATTERN.matcher(name).matches()) {
            throw new WrongStationInfoException(String.format("역 이름에 유효하지 않은 문자가 있습니다. 역 이름 : %s", name));
        }
    }
