...
public class LineName {
    ...
    private void validateNameLength(String name) {
        if (name.length() > MAX_NAME_LENGTH) {
            throw new InvalidLineNameException(String.format("노선 이름은 %d자를 초과할 수 없습니다. 이름의 길이 : %d", MAX_NAME_LENGTH, name.length()));
        }
    }
    ...
    private void validateInvalidName(String name) {
        if (!ONLY_CAN_COMPLETE_KOREAN_AND_MIDDLE_POINT_DOT_AND_ENGLISH_AND_NUMBERS_AND_PARENTHESES_AND_BLANK_PATTERN.matcher(name).matches()) {
            throw new InvalidLineNameException(String.format("노선 이름에 유효하지 않은 문자가 있습니다. 노선 이름 : %s", name));
        }
    }
